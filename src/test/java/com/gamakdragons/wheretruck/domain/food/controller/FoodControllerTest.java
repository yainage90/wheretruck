...
public class FoodControllerTest {
	...
	void testUpdate() throws Exception {
		...
		foodUpdateRequestDto.setId(foodId);
		foodUpdateRequestDto.setName("팥빵");
		foodUpdateRequestDto.setCost(1000);
		foodUpdateRequestDto.setDescription("맛있어요");
		foodUpdateRequestDto.setImage(image);

		UpdateResultDto result = UpdateResultDto.builder().result("UPDATED").id(foodId).build();

		given(foodService.updateFood(truckId, foodUpdateRequestDto)).willReturn(result);

		MockHttpServletRequestBuilder request = MockMvcRequestBuilders.multipart("/api/food/" + truckId)
																	.
																	.file(image)
																	.contentType(MediaType.MULTIPART_FORM_DATA_VALUE)
																	.param("id", foodUpdateRequestDto.getId())
																	.param("name", foodUpdateRequestDto.getName())
																	.param("cost", String.valueOf(foodUpdateRequestDto.getCost()))
																	.param("description", foodUpdateRequestDto.getDescription());
